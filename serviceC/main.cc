...
class WorkServiceImpl final : public WorkService::Service {
    Status DoWork(ServerContext* context, const WorkServiceRequest* request, WorkServiceReply* reply) override {
        std::cout << "Service C received request\n"
        ...
        std::cout << "Service C finished work\n";
        reply->set_message("Service C worked for " + std::to_string(request->duration_ms()));
        return Status::OK;
    }
};
...
void RunServer() {
    ...
    std::cout << "Service C listening on " << server_address << std::endl;
...
int main() {
    std::cout << "Hello, from service C!" << std::endl;
    RunServer();
    return 0;
}
